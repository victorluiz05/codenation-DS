...
def q3():
    pca = PCA().fit(fifa)
    coord = []
    coord.append(np.dot(x, pca.components_[0]))
    coord.append(np.dot(x, pca.components_[1]))
...
def q4():
    ...
    selected_features = (rfe.ranking_==1)
    return list(x_train.columns[selected_features])
